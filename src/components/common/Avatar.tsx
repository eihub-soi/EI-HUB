...
export const Avatar: React.FC<AvatarProps> = ({ 
  ...
  else if (size === 'lg') sizeClass = 'w-14 h-14'; // 56px
  else if (size === 'xl') sizeClass = 'w-20 h-20'; // 80px
  ...
  return (
    <img
      ...
      alt={alt}
      onError={handleError}
      className={`rounded-full object-cover flex-shrink-0 border border-white/10 ${sizeClass} ${className}`}
    />
  );
};
