...
export const InventoryManagement: React.FC = () => {
  ...
  const filteredComponents = components.filter((c) => {
    ...
    const matchesLocation = locationFilter === 'All' || c.cabinet.includes(locationFilter);
    const matchesSearch =
  ...
  const handleConfirmRestock = (e: React.FormEvent) => {
    ...
    try {
      mockEngine.restockComponent(restockComp.id, restockQty, user?.id || 'usr-faculty-1');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Manage hardware stock, cabinet shelf allocations, and restock levels</p>
        </div>
      ...
      <div className="p-4 rounded-3xl glass-card border border-white/10 flex flex-wrap items-center gap-3">
        ...
        <button className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-slate-900/60 border border-white/10 text-xs font-semibold text-slate-300 hover:text-white">
                ...
                <tr key={comp.id} className="hover:bg-slate-900/40 transition-colors">
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-3">
                      <img src={comp.image_url} alt={comp.name} className="w-9 h-9 rounded-xl object-cover" />
                      <div>
                        <p className="font-bold text-white">{comp.name}</p>
                        <p className="text-[10px] font-mono text-slate-400">{comp.sku}</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-4 px-6 text-slate-300 font-medium">{comp.category}</td>
                  <td className="py-4 px-6 font-bold text-white">{comp.total_stock}</td>
                  <td className="py-4 px-6">
                    {comp.available_stock > 0 ? (
                      <span className="font-bold text-emerald-400">{comp.available_stock}</span>
                    ) : (
                      <span className="font-bold text-rose-400">0 (Out of Stock)</span>
                    )}
                  </td>
                  <td className="py-4 px-6 font-bold text-indigo-300">{comp.borrowed_stock}</td>
                  <td className="py-4 px-6 text-slate-400">{comp.cabinet}, {comp.shelf}</td>
                  <td className="py-4 px-6 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        onClick={() => { setRestockComp(comp); setRestockQty(5); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in">
          <div className="w-full max-w-sm glass-card p-6 border border-white/20 shadow-2xl rounded-3xl space-y-4 max-h-[90vh] overflow-y-auto">
            <h3 className="text-sm font-bold text-white flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-emerald-400" /> Restock Component ({restockComp.name})
            </h3>

            <form onSubmit={handleConfirmRestock} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Units to Add to Stock</label>
                <input
                  type="number"
                  min={1}
                  value={restockQty}
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                <button type="button" onClick={() => setRestockComp(null)} className="px-4 py-2 text-slate-400 hover:text-white">
                  Cancel
                </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in">
          <div className="w-full max-w-lg glass-card p-6 border border-white/20 shadow-2xl rounded-3xl space-y-5 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                <Boxes className="w-4 h-4 text-indigo-400" /> {editingComp ? 'Edit Component' : 'Add New Component'}
              </h3>
            ...
            <form onSubmit={handleSaveComponent} className="space-y-4 text-xs">
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Component Name</label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Cabinet Rack</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Shelf / Box</label>
                  <input
                    type="text"
