import { ComponentItem, Profile, BorrowRequest, ActivityLog, NotificationItem, SystemOverviewStats, PurchaseOrder } from '../types';
import { INITIAL_COMPONENTS, INITIAL_PROFILES, INITIAL_REQUESTS, INITIAL_PURCHASE_ORDERS } from './mockData';
...
class MockEngine {
  ...
  private initStorage() {
    ...
    if (!localStorage.getItem(STORAGE_KEYS.PURCHASES)) {
      localStorage.setItem(STORAGE_KEYS.PURCHASES, JSON.stringify(INITIAL_PURCHASE_ORDERS));
    }
  }
  ...
  public addComponent(comp: Omit<ComponentItem, 'id' | 'created_at' | 'updated_at' | 'borrowed_stock' | 'available_stock'> & { available_stock?: number }): ComponentItem {
    ...
    }).then(() => this.syncWithTurso())
      .catch(err => console.error('[MockEngine] Failed to save component to backend:', err));
  ...
  public updateComponent(id: string, formData: Partial<ComponentItem>) {
    ...
    if (idx !== -1) {
      list[idx] = { ...list[idx], ...formData, updated_at: new Date().toISOString() } as ComponentItem;
      localStorage.setItem(STORAGE_KEYS.COMPONENTS, JSON.stringify(list));
    }
    ...
    apiRequest(`/api/components/${id}`, {
      ...
      .catch(err => console.error('[MockEngine] Failed to update component on backend:', err));
  ...
  public deleteComponent(id: string) {
    ...
    apiRequest(`/api/components/${id}`, {
      ...
      .catch(err => console.error('[MockEngine] Failed to delete component from backend:', err));

    this.notify();
  }

  public restockComponent(componentId: string, quantity: number, userId: string) {
    ...
    if (comp) {
      ...
      apiRequest(`/api/components/${componentId}`, {
        ...
        .catch(err => console.error('[MockEngine] Failed to restock component on backend:', err));
    }
    this.notify();
  }
  ...
  public submitBorrowRequest(studentId: string, componentId: string, quantity: number, purpose: string, days: number = 14): BorrowRequest {
    const comps = this.getComponents();
    const comp = comps.find(c => c.id === componentId);
    ...
    const newReq: BorrowRequest = {
      ...
      expected_return_at: new Date(Date.now() + days * 24 * 60 * 60 * 1000).toISOString(),
    ...
    apiRequest('/api/requests/submit', {
      ...
      .catch(err => console.error('[MockEngine] Failed to submit request to backend:', err));

    this.notify();
    return newReq;
  }

  public approveBorrowRequest(requestId: string, facultyId: string, remark?: string): BorrowRequest {
    const requests = this.getRequests();
    ...
    import('../utils/pdfGenerator').then(async ({ generateStudentReceiptPdf }) => {
      let pdfBase64 = '';
      try {
        const doc = await generateStudentReceiptPdf(req, false);
        pdfBase64 = doc.output('datauristring').split(',')[1];
      } catch (pdfErr) {
        console.warn('[MockEngine] Failed to generate PDF for email attachment:', pdfErr);
      }
      ...
      apiRequest(`/api/requests/${requestId}/approve`, {
        ...
        .catch(err => console.error('[MockEngine] Failed to approve request on backend:', err));
    }).catch(() => {
      apiRequest(`/api/requests/${requestId}/approve`, {
        method: 'POST',
        body: JSON.stringify({
  ...
  public rejectBorrowRequest(requestId: string, facultyId: string, reason: string): BorrowRequest {
    const requests = this.getRequests();
    const req = requests.find((r) => r.id === requestId);
    if (!req) throw new Error('Request not found');
    ...
    apiRequest(`/api/requests/${requestId}/reject`, {
      ...
      .catch(err => console.error('[MockEngine] Failed to reject request on backend:', err));

    this.notify();
    return req;
  }

  public requestReturnComponent(requestId: string, studentId: string, condition: string, description: string) {
    const requests = this.getRequests();
    const req = requests.find((r) => r.id === requestId);
    if (!req) throw new Error('Request not found');
    ...
    apiRequest(`/api/requests/${requestId}/return-request`, {
      ...
      .catch(err => console.error('[MockEngine] Failed to submit return request to backend:', err));
  ...
  public processReturnComponent(
    ...
    const comp = comps.find(c => c.id === req.component_id);
    if (comp) {
      comp.available_stock = Math.min(comp.total_stock, comp.available_stock + req.quantity);
    ...
    apiRequest(`/api/requests/${requestId}/return-process`, {
      ...
      .catch(err => console.error('[MockEngine] Failed to process return on backend:', err));
  ...
  public addProfile(profile: Omit<Profile, 'id' | 'created_at' | 'updated_at'> & { id?: string }, password?: string): Profile {
    ...
    apiRequest('/api/profiles/sync', {
      ...
      .catch(err => console.error('[MockEngine] Failed to add profile to backend:', err));

    this.notify();
    return newProfile;
  }

  public async updateProfile(id: string, formData: Partial<Profile>): Promise<void> {
    const list = this.getProfiles();
    const idx = list.findIndex(p => p.id === id);
    if (idx !== -1) {
      list[idx] = { ...list[idx], ...formData, updated_at: new Date().toISOString() } as Profile;
      localStorage.setItem(STORAGE_KEYS.PROFILES, JSON.stringify(list));
    }
  ...
  public addPurchaseOrder(poData: Omit<PurchaseOrder, 'id' | 'po_number' | 'status' | 'purchased_at' | 'created_at'>): PurchaseOrder {
    ...
    apiRequest('/api/purchase-orders', {
      ...
      .catch(err => console.error('[MockEngine] Failed to save purchase order to backend:', err));

    this.notify();
    return newPO;
  }

  public async updatePurchaseOrder(id: string, poData: Partial<PurchaseOrder>): Promise<void> {
    ...
    if (idx !== -1) {
      list[idx] = { 
        ...list[idx], 
        ...poData, 
        total_cost: (poData.quantity ?? list[idx].quantity) * (poData.unit_cost ?? list[idx].unit_cost), 
        updated_at: new Date().toISOString() 
      } as PurchaseOrder;
    ...
    try {
      ...
      console.error('[MockEngine] Failed to update purchase order on backend:', err);
    }
    this.notify();
  }
  ...
  public async deletePurchaseOrder(id: string): Promise<void> {
    ...
    try {
      ...
      console.error('[MockEngine] Failed to delete purchase order on backend:', err);
    }
    this.notify();
  }
  ...
  public logActivity(action: string, entity_type: string, entity_id: string, details: any, severity: string = 'info') {
    apiRequest('/api/activity-logs', {
      ...
      .catch(err => console.error('[MockEngine] Failed to log activity on backend:', err));
  }
  ...
  public markNotificationAsRead(id: string) {
    ...
    if (notif) {
      ...
      apiRequest(`/api/notifications/${id}/read`, {
        ...
        .catch(err => console.error('[MockEngine] Failed to mark notification read on backend:', err));
  ...
  public async checkForDeadlineReminders() {
    try {
      ...
      console.error('[MockEngine] Failed to run deadline reminders check on backend:', e);
    }
  }
  ...
  public getSystemStats(): SystemOverviewStats {
    ...
    const availableStock = components.reduce((acc, c) => acc + c.available_stock, 0);
    const borrowedStock = components.reduce((acc, c) => acc + c.borrowed_stock, 0);
    const lowStockItemsCount = components.filter((c) => c.available_stock > 0 && c.available_stock <= 5).length;
    const outOfStockItemsCount = components.filter((c) => c.available_stock === 0).length;

    const pendingRequestsCount = requests.filter((r) => r.status === 'pending').length;
    const pendingReturnsCount = requests.filter((r) => r.return_requested_at && r.status !== 'returned').length;
    const activeLoansCount = requests.filter((r) => r.status === 'approved' && !r.returned_at).length;
